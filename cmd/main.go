...
func main() {
	...
	if err != nil {
		logger.Error("unable to initialize ingress route controller", "error", err)
		os.Exit(1)
	}
	if err := controller.SetupWithManager(manager); err != nil {
		logger.Error("unable to start ingress route controller", "error", err)
		os.Exit(1)
	}
	...
	if err := manager.AddHealthzCheck("healthz", healthz.Ping); err != nil {
		logger.Error("unable to set up health check at /healthz", "error", err)
		os.Exit(1)
	}
	...
	if err := manager.Start(ctrl.SetupSignalHandler()); err != nil {
		logger.Error("failed to run manager", "error", err)
		os.Exit(1)
	}
