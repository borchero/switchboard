...
func (r *IngressRouteReconciler) Reconcile(
	...
	logger := r.logger.With(slog.String("name", req.String()))
	...
	if err := r.Get(ctx, req.NamespacedName, &ingressRoute); err != nil {
		if !apierrs.IsNotFound(err) {
			logger.Error("unable to query for ingress route", slog.Any("error", err))
		}
	...
	if err != nil {
		logger.Error("failed to parse hosts from ingress route", slog.Any("error", err))
	...
	for _, itg := range r.integrations {
		if !r.selector.MatchesIntegration(ingressRoute.Annotations, itg.Name()) {
			...
			logger.Debug("ignoring integration", slog.String("integration", itg.Name()))
			continue
		}
		if err := itg.UpdateResource(ctx, &ingressRoute, info); err != nil {
			logger.Error("failed to upsert resource",
				slog.String("integration", itg.Name()), slog.Any("error", err),
			)
			return ctrl.Result{}, err
		}
		logger.Debug("successfully upserted resource", slog.String("integration", itg.Name()))
	}
